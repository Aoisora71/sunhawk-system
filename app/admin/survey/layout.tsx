...
export default async function AdminSurveyLayout({
  ...
  try {
    ...
    if (payload.userId === DEV_ACCOUNT_ID && payload.email === DEV_ACCOUNT_EMAIL) {
      // Developer account - allow access to admin panel (same as admin)
      return <>{children}</>
    }

    // For regular users, check role and database
    ...
    console.error("Admin survey layout auth check error:", error)
    redirect("/login")
  }
