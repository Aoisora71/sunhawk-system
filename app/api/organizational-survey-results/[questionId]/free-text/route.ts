...
async function handlePut(
  ...
  try {
    ...
    console.log('[Organizational Survey Free Text API] Active survey check:', {
    ...
    if (!activeSurvey) {
      console.log('[Organizational Survey Free Text API] No active survey found')
      return badRequestResponse("現在、ソシキサーベイのサーベイ期間ではありません")
    }
    
    if (Number(activeSurvey.id) !== Number(surveyId)) {
      console.log('[Organizational Survey Free Text API] Survey ID mismatch:', {
    ...
    const singleChoiceQuestionIds = Array.isArray(singleChoiceResponse)
      ? new Set(singleChoiceResponse.map((item: any) => item.qid ?? item.questionId))
      : new Set()
